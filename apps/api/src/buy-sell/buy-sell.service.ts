  ...
  async create(
    ...
    try {
      ...
      if (
        ...
        (market.max_order_amount &&
          quantity > market.max_order_amount.toNumber())
      ) {
      ...
      const order = await this.prisma.$transaction(async (tx) => {
        const newAvailable =
          wallet.available_balance.toNumber() - requiredAmount;
      ...
      await this.notificationPublisher.publishUserNotification({
        ...
        message: `Lệnh ${side.toUpperCase()} ${market.symbol} đã được tạo`,
        metadata: {
          marketId: market_id,
      ...
      await this.notificationPublisher.publishUserNotification({
        ...
        message: getErrorMessage(error, 'Không thể đặt lệnh'),
        metadata: {
          marketId: createBuySellDto.market_id,
          side: createBuySellDto.side,
          type: createBuySellDto.type,
          quantity: createBuySellDto.quantity,
          price: createBuySellDto.price,
        },
      });
      throw error;
    }
